...
import { SIXTH_PHASE_TABLES_QUANT, isSixthPhaseValid } from '@shared/helpers/champion-config';
import { createEmptyArrays, distributePlayers } from '@shared/utils/functions';
import { MessageService } from 'primeng/api';
import { ButtonDirective } from 'primeng/button';

type DrawChangeDTO = Pick<StagesInterface, 'sixthPhase'>;

@Component({
	selector: 'app-champion-progress-sixth-step',
	standalone: true,
	imports: [DeskCardComponent, ButtonDirective, ChampionStageSummaryComponent],
	templateUrl: './champion-progress-sixth-step.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ChampionProgressSixthStepComponent {
	...
	private logicNextValid() {
		...
		} else if (!isSixthPhaseValid(this.gridData())) {
	...
	protected generateDraw() {
		...
		const dto: DrawChangeDTO = {
			sixthPhase: this.distributePlayers(),
		};
	...
	private distributePlayers(): TablesInterface {
		...
		const distributePlayersData = distributePlayers(playersClassified, SIXTH_PHASE_TABLES_QUANT);
